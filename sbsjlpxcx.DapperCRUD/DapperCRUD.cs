...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                ...
                default:
                    ...
                    _getPagedListSql = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY {OrderBy}) AS PagedNumber, {SelectColumns} FROM {TableName} {WhereClause}) AS u WHERE PagedNUMBER BETWEEN (({PageNumber}-1) * {RowsPerPage} + 1) AND ({PageNumber} * {RowsPerPage});Select count(1) as totalCount from {TableName} {WhereClause}";
                    break;
            }
        }
        ...
        public static IEnumerable<T> GetListPaged<T>(this IDbConnection connection, int PageIndex, int rowsPerPage, string conditions, string orderby,ref int totalCount, object parameters = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            var result = connection.QueryMultiple(query, parameters, connection.BeginTransaction(), commandTimeout);
            var result1= result.Read<T>();
            totalCount = result.Read().FirstOrDefault().totalCount;
            return result1;
        }
